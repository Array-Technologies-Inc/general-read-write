            ...
            try:
                ...
                for write_key, value in self.write_dict.items():
                    try:
                        ...
                        self.log.info(f"GW {self.gateway_ip}, TSC {self.get_id()}: Cannot set register {write_key}={value}: {e}")
                        written = False
                ...
                for mask_write_key, mask in self.mask_write_dict.items():
                    if not pd.isna(mask) and mask != 'OK':
                        ...
                        try:
                            ...
                            self.log.info(f"GW {self.gateway_ip}, TSC {self.get_id()}: Cannot set register mask {mask_write_key}={and_mask}|{or_mask}: {e}")
                            mask_written = False
