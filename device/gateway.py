...
class Gateway(Thread):
    ...
    def get_read_tsc(self) -> int:
        return self.tsc_read
    ...
    def get_read_iwc(self) -> int:
        return self.iwc_read
    ...
    def read_write_iwc(self, iwc_id: int) -> None:
        self.iwc_list[iwc_id].read_write_device()

    def read_write_all_devices(self):
        iteration = 0
        try:
        ...
        if self.client.is_socket_open():
            self.verify_iwc()
            self.verify_tsc()
            while (((self.tsc_aborted + self.tsc_full < self.get_tsc_num()) or (self.iwc_aborted + self.iwc_full < self.get_iwc_num()))):
                if not self.iwc_queue.empty():
                    iwc = self.iwc_queue.get()
                    iwc_id = iwc.get_id()
                    if not (self.iwc_list[iwc_id].get_finished() or self.iwc_list[iwc_id].get_aborted()):
                        self.iwc_list[iwc_id].client = self.client
                        self.read_write_iwc(iwc_id)
                    self.iwc_queue.task_done()
                elif not self.tsc_queue.empty():
                    tsc = self.tsc_queue.get()
                    tsc_id = tsc.get_id()
                    if not (self.tsc_list[tsc_id].get_finished() or self.tsc_list[tsc_id].get_aborted()):
                        self.tsc_list[tsc_id].client = self.client
                        self.read_write_tsc(tsc_id)
    ...
    def run(self):
        ...
        for i in range(1, self.threads_per_gateway + 1):
            if not self.process_finished:
                try:
                    gw_thread = Thread(target=self.read_write_all_devices)
